...
我目前正在我的 VPS 上运行自托管邮件服务器，并注意到出站端口 25 和 587 以及入站端口 587 似乎被阻止了。
如果可以的话，您能否帮助解除这些与邮件服务相关的端口的阻塞？

该服务器仅用于发送事务性邮件（例如论坛注册和账户通知）。我已启用 SPF、DKIM 和 DMARC 记录，发送量相对较低。不涉及批量邮件或营销邮件。

此外，我想为我的服务器 IP 请求设置 PTR 记录以提高电子邮件的送达率。  
您能否将其指向：mail.example.com
