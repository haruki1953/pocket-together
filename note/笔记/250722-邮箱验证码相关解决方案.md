...
#### 尝试发工单解决问题
原来一般服务商会禁用邮件相关端口，要发工单请求解封邮件相关端口，并设置PTR记录
```
Hello,

I’m currently running a self-hosted mail server on my VPS and noticed that outbound port 25 and 587, as well as inbound port 587, appear to be blocked.
If possible, could you please help unblock these mail service-related ports?

The server is intended for transactional emails only (such as forum registration and account notifications). I’ve implemented SPF, DKIM, and DMARC records, and the sending volume is relatively low. No bulk or marketing emails are involved.

Additionally, I’d like to request a PTR record setup for my server IP to improve email deliverability.  
Could you please point it to: mail.example.com
...
客服回复了，说需要验证域名
```
Hello haruki, to confirm your request for SMTP access, please provide a domain name you will use for your email services, and add this TXT record to it:
...
自己按要求添加TXT记录，并回复
```
Thanks for your response. I've completed the setup as requested:  

- **Domain** : example.com
- **TXT Record Name** : example.com
- **TXT Value** : buyvm-validation=......
...
We have enabled mail ports on your VM. Please be aware that our Acceptable Use Policy (AUP) strictly prohibits the sending of spam. If there are any complaints or detection of such activity, your services/account may be suspended without a refund. We advise you to use our mail service responsibly and to adhere to the terms outlined in our AUP.
